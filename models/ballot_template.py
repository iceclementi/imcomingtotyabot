...
temp_ballot_storage = dict()


class BallotTemplate(Template):
    def __init__(
        self, temp_id: str, name: str, description: str, title_format: FormatTextCode,
        description_format: FormatTextCode, options: List[str], choices: List[str], single_response: bool,
        creator_id: int
    ) -> None:
    ...
    @staticmethod
    def get_template_by_id(temp_id: str) -> BallotTemplate:
        return temp_ballot_storage.get(temp_id, None)

    @staticmethod
    def get_templates_by_ids(temp_ids: set, filters="") -> List[BallotTemplate]:
        template_ballots = [BallotTemplate.get_template_by_id(temp_id) for temp_id in temp_ids]
        return [template for template in template_ballots if filters.lower() in template.name.lower()]

    @classmethod
    def create_new(
        cls, name: str, description: str, title_format_string: str, description_format_string: str,
        options: List[str], choices: List[str], single_response: bool, creator_id: int
    ) -> BallotTemplate:
        temp_id = "L" + util.generate_random_id(models.BALLOT_ID_LENGTH, set(temp_ballot_storage.keys()))
        ...
        template = cls(
            temp_id, name, description, title_format, description_format, options, choices,
            single_response, creator_id
        )
        temp_ballot_storage[temp_id] = template
        return template

    @classmethod
    def load(
        cls, temp_id: str, name: str, description: str, title_format_data: Dict[str, Dict[str, List[str]]],
        description_format_data: Dict[str, Dict[str, List[str]]], options: List[str], choices: List[str],
        single_response: bool, creator_id: int
    ) -> None:
    ...
    def delete(self) -> None:
        temp_ballot_storage.pop(self._temp_id, None)
    ...
    def generate_linked_summary(self, include_creator=False) -> str:
        header = f"<b>{self.name} {models.EMOJI_BALLOT}</b>"
        if self.description:
    ...
    def render_text(self) -> str:
        header = f"<b>{models.EMOJI_BALLOT} {self.name} (Template)</b>"
        if self.description:
            header += f"\n<i>{self.description}</i>"
    ...
    def build_main_buttons(self) -> InlineKeyboardMarkup:
        generate_ballot_button = self.build_button("Generate Ballot", models.BALLOT)
        ...
        buttons = [[generate_ballot_button], [settings_buttons], [refresh_button, close_button]]
    ...
    def build_settings_buttons(self, is_creator=False) -> InlineKeyboardMarkup:
        ...
        edit_title_button = self.build_button("Edit Title", f"{models.EDIT}_{models.TITLE}")
        edit_description_button = self.build_button("Edit Description", f"{models.EDIT}_{models.DESCRIPTION}")
        edit_options_button = self.build_button("Edit Options", f"{models.EDIT}_{models.OPTIONS}")
        edit_choices_button = self.build_button("Edit Choices", f"{models.EDIT}_{models.CHOICES}")
    ...
    def build_button(self, text: str, action: str) -> InlineKeyboardButton:
        return util.build_button(text, models.TEMP_BALLOT_SUBJECT, action, self.temp_id)

    def to_json(self) -> dict:
        return {
            db.TEMP_BALLOT_ID:                 self.temp_id,
            db.TEMP_BALLOT_NAME:               self.name,
            db.TEMP_BALLOT_DESCRIPTION:        self.description,
            db.TEMP_BALLOT_TITLE_FORMAT:       self.title_format.to_json(),
            db.TEMP_BALLOT_DESCRIPTION_FORMAT: self.description_format.to_json(),
            db.TEMP_BALLOT_OPTIONS:            self.options,
            db.TEMP_BALLOT_CHOICES:            self.choices,
            db.TEMP_BALLOT_SINGLE_RESPONSE:    self.is_single_response,
            db.TEMP_BALLOT_CREATOR_ID:         self.creator_id,
        }
