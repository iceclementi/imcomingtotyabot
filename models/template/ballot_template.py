...
class BallotTemplate(Template):
    ...
    def create_new(
        ...
        temp_id = "B" + util.generate_random_id(const.BALLOT_ID_LENGTH, set(temp_ballot_storage.keys()))
    ...
    def generate_linked_summary(self, include_creator=False) -> str:
        header = f"<b>{self.name} {const.EMOJI_BALLOT}</b>"
        if self.description:
    ...
    def render_text(self) -> str:
        header = f"<b>{const.EMOJI_BALLOT} {self.name} (Template)</b>"
        if self.description:
            header += f"\n<i>{self.description}</i>"
    ...
    def build_main_buttons(self) -> InlineKeyboardMarkup:
        generate_ballot_button = self.build_button("Generate Ballot", const.BALLOT)
    ...
    def build_settings_buttons(self, is_creator=False) -> InlineKeyboardMarkup:
        ...
        edit_title_button = self.build_button("Edit Title", f"{const.EDIT}_{const.TITLE}")
        edit_description_button = self.build_button("Edit Description", f"{const.EDIT}_{const.DESCRIPTION}")
        edit_options_button = self.build_button("Edit Options", f"{const.EDIT}_{const.OPTIONS}")
        edit_choices_button = self.build_button("Edit Choices", f"{const.EDIT}_{const.CHOICES}")
    ...
    def build_button(self, text: str, action: str) -> InlineKeyboardButton:
        return util.build_button(text, const.TEMP_BALLOT_SUBJECT, action, self.temp_id)
