...
class Group(object):
    ...
    def create_new(cls, name: str, uid: int, password="") -> Group:
        gid = util.generate_random_id(const.GROUP_ID_LENGTH, set(group_storage.keys()))
        group = cls(gid, name, uid, password, {uid}, set(), set(), set(), datetime.now(tz=const.tz))
    ...
    def add_member(self, uid: int) -> str:
        ...
        if len(self._member_ids) >= const.MAX_GROUP_SIZE:
            return f"The group size limit ({const.MAX_GROUP_SIZE}) has been reached."
    ...
    def get_ballots(self, filters="") -> List[Ballot]:
        group_ballots = Ballot.get_ballots_by_ids(self._ballot_ids, filters)
        return sorted(group_ballots, key=lambda _list: _list.get_created_date(), reverse=True)
    ...
    def generate_linked_summary(self, include_creator=False) -> str:
        header = f"<b>{self._name[:60]}</b> ({len(self._member_ids)} {const.EMOJI_PEOPLE})"
        link = f"/group_{self._gid}"
        creator = f"{const.EMOJI_CROWN} {User.get_user_by_id(self._owner).get_name()}"
    ...
    def generate_group_members_list(self) -> str:
        ...
        for member in self.get_members():
            if member.get_uid() == self._owner:
                member_name = f"{util.make_html_bold(member.get_name())} {const.EMOJI_CROWN}"
            else:
    ...
    def generate_group_description_summary(self) -> str:
        owner = [f"{const.EMOJI_CROWN} {User.get_user_by_id(self._owner).get_name()}"]
        member_count = [f"{const.EMOJI_PEOPLE} {len(self._member_ids)}"]
        poll_count = [f"{const.EMOJI_POLL} {len(self._poll_ids)}"]
        list_count = [f"{const.EMOJI_BALLOT} {len(self._ballot_ids)}"]
    ...
    def render_group_details_text(self) -> str:
        ...
        poll_count = f"{const.EMOJI_POLL} {len(self._poll_ids)}"
        list_count = f"{const.EMOJI_BALLOT} {len(self._ballot_ids)}"
        template_count = f"{const.EMOJI_TEMPLATE} {len(self._template_ids)}"
    ...
    def render_group_members_text(self) -> str:
        ...
        footer = f"{const.EMOJI_PEOPLE} {footer_description}"
    ...
    def render_group_polls_text(self) -> str:
        ...
        footer = f"{const.EMOJI_POLL} {footer_description}"
    ...
    def render_group_ballots_text(self) -> str:
        ...
        footer = f"{const.EMOJI_BALLOT} {footer_description}"
    ...
    def render_group_templates_text(self) -> str:
        ...
        footer = f"{const.EMOJI_TEMPLATE} {footer_description}"
    ...
    def build_main_buttons(self) -> InlineKeyboardMarkup:
        view_button = self.build_button("View", const.VIEW)
    ...
    def build_view_buttons(self) -> InlineKeyboardMarkup:
        view_members_button = self.build_button("View Members", const.MEMBER)
        view_polls_button = self.build_button("View Polls", const.POLL)
        view_ballots_button = self.build_button("View Ballots", const.BALLOT)
        view_templates_button = self.build_button("View Templates", const.TEMPLATE)
    ...
    def build_view_members_buttons(self, is_owner=False) -> InlineKeyboardMarkup:
        back_button = self.build_button("Back", const.VIEW)
        if not is_owner:
        ...
        if len(self._member_ids) > 1:
            remove_member_button = self.build_button("Remove Member", f"{const.DELETE}_{const.MEMBER}")
            buttons.append([remove_member_button])

        buttons.append([back_button])
        return InlineKeyboardMarkup(buttons)

    def build_members_buttons(self, member_action: str, back_action=const.MEMBER) -> InlineKeyboardMarkup:
        buttons = []
        for member in self.get_members():
            if member.get_uid() != self._owner:
    ...
    def build_view_polls_buttons(self, user_polls: List[Poll], is_owner=False) -> InlineKeyboardMarkup:
        add_group_poll_button = self.build_button("Add Group Poll", f"{const.ADD}_{const.POLL}")
        remove_group_poll_button = self.build_button("Remove Group Poll", f"{const.DELETE}_{const.POLL}")
        back_button = self.build_button("Back", const.VIEW)
    ...
    def build_add_polls_buttons(self, user_polls: List[Poll]) -> InlineKeyboardMarkup:
        ...
        if not polls:
            return self.build_single_back_button(const.POLL)

        buttons = [[self.build_button(poll.get_title(), f"{const.ADD}_{const.POLL}_{poll.get_poll_id()}")]
                   for poll in polls]
        back_button = self.build_button("Back", const.POLL)
        buttons.append([back_button])
        return InlineKeyboardMarkup(buttons)
    ...
    def build_remove_polls_buttons(self, user_polls: List[Poll], is_owner=False) -> InlineKeyboardMarkup:
        ...
        buttons = [[self.build_button(poll.get_title(), f"{const.DELETE}_{const.POLL}_{poll.get_poll_id()}")]
                   for poll in polls]
        back_button = self.build_button("Back", const.POLL)
        buttons.append([back_button])
        return InlineKeyboardMarkup(buttons)

    def build_view_ballots_buttons(self, user_ballots: List[Ballot], is_owner=False) -> InlineKeyboardMarkup:
        add_group_ballot_button = self.build_button("Add Group Ballot", f"{const.ADD}_{const.BALLOT}")
        remove_group_ballot_button = self.build_button("Remove Group Ballot", f"{const.DELETE}_{const.BALLOT}")
        back_button = self.build_button("Back", const.VIEW)
        show_remove_button = len(self._ballot_ids) and (
                    is_owner or any(ballot.get_ballot_id() in self.get_ballot_ids() for ballot in user_ballots))
    ...
    def build_add_ballots_buttons(self, user_ballots: List[Ballot]) -> InlineKeyboardMarkup:
        ballots = [ballot for ballot in user_ballots if ballot.get_ballot_id() not in self.get_ballot_ids()]

        if not ballots:
            return self.build_single_back_button(const.BALLOT)

        buttons = [[self.build_button(ballot.get_title(), f"{const.ADD}_{const.BALLOT}_{ballot.get_ballot_id()}")]
                   for ballot in ballots]
        back_button = self.build_button("Back", const.BALLOT)
        buttons.append([back_button])
        return InlineKeyboardMarkup(buttons)

    def build_remove_ballots_buttons(self, user_ballots: List[Ballot], is_owner=False) -> InlineKeyboardMarkup:
        if is_owner:
            ballots = self.get_ballots()
        else:
            ballots = [ballot for ballot in user_ballots if ballot.get_ballot_id() in self.get_ballot_ids()]

        if not ballots:
            return self.build_single_back_button(const.BALLOT)

        buttons = [[self.build_button(ballot.get_title(), f"{const.DELETE}_{const.BALLOT}_{ballot.get_ballot_id()}")]
                   for ballot in ballots]
        back_button = self.build_button("Back", const.BALLOT)
        buttons.append([back_button])
        return InlineKeyboardMarkup(buttons)
    ...
    def build_view_templates_buttons(self, user_templates: List[Template], is_owner=False) -> InlineKeyboardMarkup:
        add_group_template_button = self.build_button("Add Group Template", f"{const.ADD}_{const.TEMPLATE}")
        remove_group_template_button = self.build_button("Remove Group Template", f"{const.DELETE}_{const.TEMPLATE}")
        back_button = self.build_button("Back", const.VIEW)
    ...
    def build_add_templates_buttons(self, user_templates: List[Template]) -> InlineKeyboardMarkup:
        ...
        if not templates:
            return self.build_single_back_button(const.TEMPLATE)

        buttons = [
            [self.build_button(
                f"{template.icon} {template.name}", f"{const.ADD}_{const.TEMPLATE}_{template.temp_id}"
            )]
            for template in templates
        ]

        back_button = self.build_button("Back", const.TEMPLATE)
        buttons.append([back_button])
        return InlineKeyboardMarkup(buttons)
    ...
    def build_remove_templates_buttons(self, user_templates: List[Template], is_owner=False) -> InlineKeyboardMarkup:
        ...
        buttons = [
            [self.build_button(
                f"{template.icon} {template.name}", f"{const.DELETE}_{const.TEMPLATE}_{template.temp_id}"
            )]
            for template in templates
        ]
        back_button = self.build_button("Back", const.TEMPLATE)
    ...
    def build_settings_buttons(self, is_owner=False) -> InlineKeyboardMarkup:
        if is_owner:
            change_name_button = self.build_button("Change Group Name", f"{const.RENAME}_{const.NAME}")
            change_password_button = self.build_button("Change Password", const.SECRET)
            delete_group_button = self.build_button("Delete Group", const.DELETE)
            buttons = [[change_name_button], [change_password_button], [delete_group_button]]
        else:
            leave_group_button = self.build_button("Leave Group", const.LEAVE_GROUP)
    ...
    def build_button(self, text: str, action: str) -> InlineKeyboardButton:
        return util.build_button(text, const.GROUP_SUBJECT, action, self._gid)
